...
class On extends Base {
  ...
  _middleware (event, context, resolve, reject) {
    ...
    const _checkImplicitNext = (newResult, nextCalled, context, nextOnce) =>
      newResult === undefined && !nextCalled && !this._replyRejectCalled(context) ? nextOnce() : newResult

    // in case no result was returned propagate the result of the next handler
    const _propagateResult = (result, newResult) => (newResult !== undefined ? newResult : result)
      ...
      while (i < length) {
        ...
        if (this._match(event, context, handler)) {
          ...
          try {
            ...
            newResult = _checkImplicitNext(newResult, nextCalled, context, nextOnce)

            result = _propagateResult(result, newResult)
