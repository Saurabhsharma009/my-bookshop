...
function transformForHana(inputModel, options) {
  ...
  function ignoreAssociationToSkippedTarget(member) {
    if(!member._ignore && isAssocOrComposition(member.type) && isNoDbArtifact(member.target._artifact)) {
      signal(info`"${member.name.absolute}": Removing ${member.type._artifact.name.absolute.replace('cds.', '')} "${member.name.element || member.name.id}", Target "${member.target._artifact.name.absolute}" is either 'abstract' or annotated with '@cds.persistence.skip'`, member.location);
      member._ignore = true;
    }
  }
