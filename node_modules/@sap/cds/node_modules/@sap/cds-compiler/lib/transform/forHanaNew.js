...
function transformForHanaWithCsn(inputModel, options) {
  ...
  const addDefaultTypeFacets = transformUtils.getTransformers(csn, pathDelimiter).addDefaultTypeFacets;
  const _toFinalBaseType = transformUtils.getTransformers(csn, pathDelimiter).toFinalBaseType;
  ...
  const { flattenStructuredElement, checkForeignKeys,
    ...
    recurseElements, toFinalBaseType
  ...
  forEachDefinition(csn, (artifact, artifactName) => {
    if ((artifact.kind == 'entity' || artifact.kind == 'view') && hasBoolAnnotation(artifact, '@odata.draft.enabled')) {
      ...
      for (let name in draftNodes) {
        generateDraftForHana(draftNodes[name], name);
      }

      if(draftNodes){
        if(!csn.definitions['DRAFT'] && csn.definitions['DRAFT.DraftAdministrativeData']){
          csn.definitions['DRAFT'] = { kind: 'context' };
      ...
      if(query.SET){
        ...
        if(query.SELECT.from.args){
          ...
          for(const subQuery of query.SELECT.from.args){
            if(subQuery.ref){
              const art = _artifactRef(subQuery);
  ...
  */
  ...
  function transformCommon(obj, objName, path) {
    ...
    */
    ...
    if (!obj._ignore && obj.type) {
      checkTypeParameters(obj, objName, csn, path);
    }
    if (!obj._ignore && obj.items && obj.items.type) {
      checkTypeParameters(obj.items, objName, csn, path.concat(['items']));
    }
  ...
  function generateDraftForHana(artifact, artifactName) {
    ...
    if (!options.forHana.keepStructsAssocs && draftUUIDKey) {
      createForeignKeyElement(draftAdministrativeData.DraftAdministrativeData, 'DraftAdministrativeData', draftUUIDKey, draftsArtifact, draftsArtifactName);
   ...
   * 
   * @param {any} artifactName Absolute name of the artifact
   * @returns {String} Name of the service or false.
   */
  ...
  function basicFlattening(art, artName){
    ...
    if (!options.forHana.keepStructsAssocs) {
      ...
      forEachGeneric(art, 'elements', (element, elemName) => {
        ...
        if (element.viaTransform && element.type && isAssocOrComposition(element.type, csn)) {
          if(element.on){
    ...
    if (tc && tc.fzindexes) {
      tc.fzindexes.forEach(idx => {
        idx.columns.forEach(col => {
          if (!col._ignore) {
            setProp(art.elements[col.path[0].id], '_fzindex', idx); // last one wins on multiple occurrences
          }
        });
      });
    }
        ...
        if (isManagedAssociationElement(elem)) {
          ...
          if (!elem.query) {
            for (let name in elem.keys) {
              ...
              if (!foreignKey.calculated && !foreignKey.$inferred && !foreignKey.$inferredAlias && foreignKey.as) {
